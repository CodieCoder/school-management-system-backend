...
module.exports = class StudentManager {
    ...
    async getStudent({ __auth, __query }) {
        ...
        const student = await Student.findById(studentId).populate('classroomId', 'name').lean();
    ...
    async updateStudent({ __auth, studentId, name, email, classroomId }) {
        ...
        if (email !== undefined) {
            if (email) {
                const dup = await Student.findOne({ email: email.toLowerCase(), _id: { $ne: student._id } });
                if (dup) return { error: 'student email already exists' };
            }
            student.email = email;
        }
