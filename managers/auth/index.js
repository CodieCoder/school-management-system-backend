...
module.exports = class AuthManager {
    ...
    async login({ email, password }) {
        ...
        const user = await this.userManager.getByAuthId({ authId: authResult.authId });
        if (!user) return { error: 'user not found' };

        const memberships = await this.schoolMembership.getMemberships({ userId: user._id });
    ...
    async seedSuperAdmin() {
        ...
        const existing = await require('./auth_identity.mongoModel').findOne({ email: email.toLowerCase() });
        if (existing) return;

        const authResult = await this.adapter.register({ email, password });
        if (authResult.error) return;

        const user = await User.create({ authId: authResult.authId, displayName: 'Super Admin' });
        const superRole = await Role.findOne({ name: 'superadmin', schoolId: null });
        if (superRole) {
            await this.schoolMembership.createGlobal({ userId: user._id, roleId: superRole._id });
        }
