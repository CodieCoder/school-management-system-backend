...
module.exports = class ClassroomManager {
    ...
    _resolveSchoolId(__auth, schoolId) {
        ...
        if (schoolMemberships.length === 1) return schoolMemberships[0].schoolId.toString();
        return null;
    }
    ...
    async createClassroom({ __auth, name, schoolId, capacity, resources }) {
        ...
        if (existing) return { error: 'classroom name already exists in this school' };
    ...
    async getClassroom({ __auth, __query }) {
        ...
        if (!this.role.hasPermission(__auth, classroom.schoolId, 'classroom:read')) {
            return { error: 'permission denied' };
        }
    ...
    async updateClassroom({ __auth, classroomId, name, capacity, resources }) {
        ...
        if (!this.role.hasPermission(__auth, classroom.schoolId, 'classroom:update')) {
            return { error: 'permission denied' };
        }

        if (name !== undefined) {
            const dup = await Classroom.findOne({ schoolId: classroom.schoolId, name, _id: { $ne: classroom._id } });
    ...
    async deleteClassroom({ __auth, classroomId }) {
        ...
        if (!this.role.hasPermission(__auth, classroom.schoolId, 'classroom:delete')) {
            return { error: 'permission denied' };
        }

        await Student.updateMany({ classroomId: classroom._id }, { $set: { classroomId: null } });
