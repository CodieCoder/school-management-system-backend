...
describe('Role CRUD', () => {
    ...
    it('should create a custom role for a school', async () => {
        const res = await request
            ...
            .send({
                schoolId,
                name: 'editor',
                permissions: ['school:read', 'classroom:read', 'classroom:update', 'student:read', 'student:update'],
            });
    ...
    it('should update a custom role', async () => {
        const res = await request
            ...
            .set('token', adminToken)
            .send({ roleId: customRoleId, permissions: ['school:read', 'classroom:read'] });

        expect(res.status).toBe(200);
        expect(res.body.ok).toBe(true);
        expect(res.body.data.permissions).toEqual(['school:read', 'classroom:read']);
    });
