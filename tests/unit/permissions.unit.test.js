...
describe('RoleManager.can()', () => {
    ...
    test('multiple permissions — one matching is enough', () => {
        expect(role.can(['classroom:read', 'student:read'], 'student:read')).toBe(true);
    });
});
...
describe('RoleManager.hasPermission()', () => {
    ...
    test('denies permission the user lacks for the target school', () => {
        expect(role.hasPermission(authContext, schoolA, 'school:delete')).toBe(false);
    });

    test('permissions are school-scoped — schoolB perms don\'t apply to schoolA', () => {
        expect(role.hasPermission(authContext, schoolA, 'student:read')).toBe(false);
    });

    test('resource wildcard works within school scope', () => {
        expect(role.hasPermission(authContext, schoolB, 'student:delete')).toBe(true);
    });

    test('denies when user has no membership for the school', () => {
        expect(role.hasPermission(authContext, '64a000000000000000000099', 'school:read')).toBe(false);
    });
